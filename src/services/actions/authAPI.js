...
import {apiLoginURL, apiRefreshTokenURL, apiRegistraterURL} from "../constants/routes/authURL.js";
...
export const userRegistrationFromAPI = async (userData) => {
    try {
        const {data} = await apiRoute.post(apiRegistraterURL, userData)
        return data
    } catch (error) {
        console.error("Error logging user:", error)
        throw new Error("Failed to login user from the API.")
